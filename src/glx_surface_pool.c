           ...
           DFBX11           *x11 )
{
     ...
     if (!local->BindTexImageEXT) {
          printf( "glXGetProcAddress( 'glXBindTexImageEXT' ) failed!\n" );
     ...
     if (!local->ReleaseTexImageEXT) {
          printf( "glXGetProcAddress( 'glXReleaseTexImageEXT' ) failed!\n" );
          return DFB_UNSUPPORTED;
     }
                ...
                LocalPixmap           **ret_pixmap )
{
     ...
     if (!pixmap) {
          ...
          if (!pixmap->drawable) {
               printf( "GLX/Surfaces: Could not create %dx%d (depth %d) GLXPixmap!\n",
...
static int
glxDestroyPool( /*CoreSurfacePool*/void *pool,
                void            *pool_data,
                void            *pool_local )
{
//     D_DEBUG_AT( GLX_Surfaces, "%s()\n", __FUNCTION__ );
...
static int
glxLeavePool(/* CoreSurfacePool*/void *pool,
              void            *pool_data,
              void            *pool_local )
{
    // D_DEBUG_AT( GLX_Surfaces, "%s()\n", __FUNCTION__ );
...
static int
glxTestConfig(/* CoreSurfacePool     */void    *pool,
               void                    *pool_data,
               void                    *pool_local,
...
static int
glxAllocateBuffer(/* CoreSurfacePool */void      *pool,
                   void                  *pool_data,
                   void                  *pool_local,
                   /*CoreSurfaceBuffer*/void     *buffer,
                   /*CoreSurfaceAllocationi*/void *allocation,
                   void                  *alloc_data )
{
#if 0
     CoreSurface          *surface;
     ...
     alloc->pixmap = XCreatePixmap( local->display, DefaultRootWindow( local->display ),
                                    surface->config.size.w, surface->config.size.h, alloc->depth );
     if (!alloc->pixmap) {
          printf( "GLX/Surfaces: Could not create %dx%d (depth %d) pixmap!\n",
                   surface->config.size.w, surface->config.size.h, alloc->depth );
...
static int
glxDeallocateBuffer(/* CoreSurfacePool*/void       *pool,
                     void                  *pool_data,
                     void                  *pool_local,
                     /*CoreSurfaceBuffer */void    *buffer,
                     /*CoreSurfaceAllocation*/void *allocation,
...
static int
glxLock(/* CoreSurfacePool  */ void    *pool,
         void                  *pool_data,
         void                  *pool_local,
         /*CoreSurfaceAllocation*/void *allocation,
         void                  *alloc_data,
         /*CoreSurfaceBufferLock*/void *lock )
{
     ...
     if (lock->accessor == CSAID_GPU) {
          ...
          if (!ctx) {
               ...
               if (!ctx->context) {
                    printf( "GLX: Could not create GLXContext!\n" );
                    XUnlockDisplay( local->display );
                    D_FREE( ctx );
                    return DFB_FAILURE;
               }
...
static int
glxUnlock(/* CoreSurfacePool*/void       *pool,
           void                  *pool_data,
           void                  *pool_local,
          /* CoreSurfaceAllocation*/void *allocation,
           void                  *alloc_data,
...
static int
glxRead(/* CoreSurfacePool*/ void      *pool,
         void                  *pool_data,
         void                  *pool_local,
/*         CoreSurfaceAllocation*/void *allocation,
         void                  *alloc_data,
         ...
         /*const DFBRectangle */ void   *rect )
{
     ...
     if (!image) {
          printf( "GLX/Surfaces: XCreateImage( %dx%d, depth %d ) failed!\n", rect->w, rect->h, alloc->depth );
          XUnlockDisplay( local->display );
          return DFB_FAILURE;
     }
     ...
     if (!sub) {
          printf( "GLX/Surfaces: XGetSubImage( %d,%d-%dx%d ) failed!\n", DFB_RECTANGLE_VALS(rect) );
          return DFB_FAILURE;
     }
#endif
     return 0;
...
static int
glxWrite( /*CoreSurfacePool*/void       *pool,
          void                  *pool_data,
          void                  *pool_local,
          /*CoreSurfaceAllocation*/void *allocation,
          void                  *alloc_data,
