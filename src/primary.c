...
DFBX11Shared gShared;


/**********************************************************************************************************************/

static int
dfb_x11_create_window( DFBX11 *x11 )
{
     int           ret;
...
*/
	...
	//try to mofify pixel data
	DebugLog("rect.h = %d rect.w = %d",rect.h,rect.w);
	DebugLog("ximage->bytes_per_line = %d\n",ximage->bytes_per_line);
	unsigned char *pData = xw->virtualscreen;
	{
		for(int i = 0; i < rect.h; i++ ){ // per line
			char	*line = pData + i * rect.w * 4;
...
*/
               default:
                    printf( "unsupported depth %d", xw->depth );
          }
     }
...
/******************************************************************************/
int
dfb_x11_create_window_handler( DFBX11 *x11 )
{
     XWindow      *xw;
     DFBX11Shared *shared ;
	shared = &gShared;
     ...
     int bSucces = dfb_x11_open_window( x11, &xw, 0, 0,800,600/*, config->width, config->height, config->format*/ );

     /* Set video mode */
     if ( !bSucces ) {
          printf( "DirectFB/X11: Couldn't open  window!\n" );
...
int
dfb_x11_set_palette_handler( DFBX11 *x11 )
{
     return 0;
}
