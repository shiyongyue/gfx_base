...
{
     if (use_shm) {
          printf( "X11/Display: Error! Disabling XShm.\n" );
...
//     D_INFO( "X11/Window: Creating %4dx%4d %s window...\n", iWidth, iHeight, dfb_pixelformat_name(format) );
	 ...
	 TraceLog("xw->width = %d xw->height = %d display = %p",xw->width,xw->height,xw->display);

     xw->screenptr = DefaultScreenOfDisplay(xw->display);
	TraceLog("xw->screenptr = %p",xw->screenptr);
     xw->screennum = DefaultScreen(xw->display);
	TraceLog("xw->screennum = %d",xw->screennum);
     ...
     xw->visual    = x11->visuals[1] ?: DefaultVisualOfScreen(xw->screenptr);
	 DebugLog("xw->visual = %p",xw->visual);
#if 0
     attr.event_mask = 
            ButtonPressMask
          | ButtonReleaseMask
     ...
     XStoreName(xw->display,xw->window,"shiyy test libx11 window");
     ...
     if (x11->use_shm) {
          // Shared memory 	
          xw->shmseginfo=(XShmSegmentInfo *)calloc(1, sizeof(XShmSegmentInfo));
          if (!xw->shmseginfo) {
               x11->use_shm = 0;
               goto no_shm;
          }
          ...
          if (!xw->ximage || error_code) {
               ErrLog("X11: Error creating shared image (XShmCreateImage) \n");
               x11->use_shm = 0;
               free(xw->shmseginfo);
               error_code = 0;
               goto no_shm;
...
no_shm:
     if (!x11->use_shm) {
          ...
          XSync( xw->display, False );
          if (!xw->ximage || error_code) {
               printf( "X11/Window: XCreateImage( Visual %02lu, depth %d, size %dx%d, buffer %p [%d] ) failed!\n",
